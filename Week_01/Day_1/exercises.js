...
const longest = (a, b) => {
  ...
  for (let i in newArray) {
    if (!cleanArray.includes(newArray[i])) {
...
const myReplace = (string, toReplace, replacedWith) => {
  let returnedString = string.replace(
    toReplace,
    replacedWith.charAt(0).toUpperCase() + replacedWith.slice(1)
  );
...
const confirmEnding = (str, x) =>
  str.substring(str.length, str.length - x.length) === x
    ? console.log(true)
    : console.log(false);
