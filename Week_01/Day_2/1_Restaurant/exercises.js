...
class Dish {
    ...
    cost() {
        let totalCost = 10
        this.ingredients.forEach((currentValue) => totalCost = totalCost + currentValue.price)
        return totalCost
    }
    profit() {
...
class Restaurant{
    ...
    printOrders(){this.orderArray.forEach((currentValue, index) => console.log(`Order #${index}: ${currentValue.name}`))}
    printCheck(customer){
        console.log(customer.name)
        
        this.orderArray.forEach((currentValue, index) => {
            if (currentValue.customerId === customer.id){console.log(`Order #${index}: ${currentValue.name} - ${currentValue.price}`)}
        })
        let total = 0
    ...
    totalProfit(){
        let totalProfit = 0
        this.orderArray.forEach((currentValue) => {totalProfit +=  currentValue.profit()})
