    ...
    .then((data) => {
      data.results.forEach((movies) => {
        ...
        newA.addEventListener("click", (event) => {
          ...
          // Movie URL
          const clickedMovieUrl =
            event.explicitOriginalTarget.attributes["data-url"].nodeValue;
                ...
                fetch(starshipUrl)
                  ...
                  .then((starship) => {
                    ...
                    newA.addEventListener("click", (event) => {
                      ...
                      removeAllChildNodes(pilotsDetailList);
                      // starship URL
                      const clickedStarshipUrl =
                        event.explicitOriginalTarget.attributes["data-url"]
                          .nodeValue;
                      // fetch movieUrl to get starshipUrls
                          ...
                          if (starship.pilots.length !== 0) {
                            ...
                            starship.pilots.forEach((pilotUrl) => {
                              fetch(pilotUrl)
                                ...
                                .then((pilot) => {
                                  const newListObject =
                                    document.createElement("li"); // create listobject
                                  const newA = document.createElement("a"); // create a object
                                  newA.innerText = pilot.name; // append title to listobject text
                                  newA.href = "#";
                                  const att =
                                    document.createAttribute("data-url"); // Create a "data-url" attribute
                                  att.value = `${pilot.url}`;
                                  newA.setAttributeNode(att);
                                  // adding pilots infos *********************************************************************************
                                  newA.addEventListener("click", (event) => {
                                    ...
                                    removeAllChildNodes(pilotsDetailList);
                                    // starship URL
                                    const clickedPilotUrl =
                                      event.explicitOriginalTarget.attributes[
                                        "data-url"
                                      ].nodeValue;
                                    // fetch movieUrl to get starshipUrls
